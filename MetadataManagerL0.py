...
class NotIntegerError(ValueError):
    """used for setRating"""
    pass
class IncorrectVersionFormatError(ValueError):
    """used for setVersion"""
    pass
class UnsupportedOperationError(ValueError):
    """This operation cannot be done for this type of metadata"""
    pass
...
g_translaters = {'Exif.Image.XPTitle': raw_to_cleanStr,
                 ...
                 'Exif.Image.ImageDescription': valTranslateNone,
                 ...
                 'Xmp.taggerMark.mdEditorVersion': valTranslateNone
                 }
...
def isVersionNum(p_vers):
    """
	Version number is stored as a string. So we need to check it's format
	Proper format is X.XX
	(examples: 0.03, 1.06, 23.00)
    :param p_vers: version number
	:type p_vers: string

    :raise IncorrectVersionFormatError: if any format error is found in the version number
	Format rules:
	1. The version number must contain exactly one dot.
	2. Exactly two characters should exist after the dot. One more more must exist before the dot.
	3. Every character other than the dot must be numeric.
    """
    if p_vers.count('.')==1:
        f_a = p_vers.split('.', maxsplit=1)[0]
        f_b = p_vers.split('.', maxsplit=1)[1]
        if len(f_b)==2 and len(f_a)>0:
            if f_a.isnumeric() and f_b.isnumeric():
                #print("f_a \"", f_a, "\"", sep='')
                return
            else:
                raise IncorrectVersionFormatError("\'{}\' is not numeric".format(p_vers))
        else:
            raise IncorrectVersionFormatError("\'{}\' is not the correct length (2)".format(
        f_b))
    else:
        raise IncorrectVersionFormatError("\'{}\' does not have exactly one dot".format(p_vers))
...
def getTitle(p_filename):
    ...
    """
    filecheck(p_filename)
    if getExtension(p_filename) == '.jpg' or getExtension(p_filename) == '.png' or getExtension(p_filename) == '.tif':
...
def setVersionNum(p_filename, p_setVersionNumToThis):
	...
	:type p_setVersionNumToThis: string
	example: to set picture to version 1.03, we use 
	setVersionNum(picture, "1.02")

    :raise IncorrectVersionFormatError: an improperly formated version number for p_setVersionNumToThis
    :raise UnknownFiletypeError: if the filetype cannot be found
    :raise UnsupportedFiletypeError: if the filetype is not .jpg, .png, or .gif
    """
    filecheck(p_filename)
